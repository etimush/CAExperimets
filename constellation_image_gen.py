...
img_base = style_loss.image_loader("2butt.jpg", [HEIGHT,WIDTH])
zero_out = img_base.sum(2) == 3
img_base = img_base * ~zero_out[:,:,None]
...
nca = IsoGrowCA(CHANNELS, hidden_n=158)
#nca.load_state_dict(torch.load("./style_nca19.pth"))
nca = nca.to(DEVICE)
...
rand_p = np.array([[30,30,44,35,35,47],[27,35,30,71,78,75],])
...
np.save("constellation17", rand_p)
#with torch.no_grad():
...
for i in range(2000000 + 1):
    ...
    for j in range(513):
        ...
        if (j % 64 == 0) and (j != 0) :
            ...
            x_up = x_up[:,:,PADDING:x.shape[-2]-PADDING,PADDING:x.shape[-1]-PADDING]
    ...
    if i % 100 == 0:
        ...
        torch.save(nca.state_dict(), "style_nca19.pth")
